    ...
    <div className="flex flex-col gap-14">
      ...
      <Dialog.Root open={!!active} onOpenChange={o => !o && setActiveSlug(null)}>
        <Dialog.Portal>
          ...
          <Dialog.Popup className="fixed left-1/2 top-1/2 z-50 w-[calc(100%-2rem)] max-w-2xl -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-card max-h-[85vh] overflow-y-auto">
            {active && (
              <div className="p-6 sm:p-8 flex flex-col gap-7">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-zinc-100 text-muted-foreground">{active.category}</span>
                    <Dialog.Title className="text-2xl sm:text-3xl font-black text-foreground mt-3 leading-tight">{active.name}</Dialog.Title>
                    {active.full_name && <p className="text-sm text-muted-foreground mt-1">{active.full_name}</p>}
                  </div>
                  <Dialog.Close className="p-1.5 rounded-lg cursor-pointer text-zinc-400 hover:text-muted-foreground hover:bg-zinc-100 shrink-0">
                    <X className="h-5 w-5" />
                  </Dialog.Close>
                </div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    {active.application_window && (
                      <div className="rounded-xl border border-border p-4">
                        ...
                        <p className="text-sm text-muted-foreground whitespace-pre-line">{active.application_window}</p>
                      </div>
                    )}
                    {active.exam_dates && (
                      <div className="rounded-xl border border-border p-4">
                        ...
                        <p className="text-sm text-muted-foreground whitespace-pre-line">{active.exam_dates}</p>
                      </div>
                    )}
                  </div>
                )}
