    ...
    <div>
          ...
          <div className="flex flex-wrap items-start justify-between gap-6">
            <div className="max-w-2xl">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-zinc-100 text-zinc-600">{college.type}</span>
                {college.nirf_rank && (
              ...
              <h1 className="text-4xl sm:text-5xl font-black tracking-tight text-zinc-900 leading-[1.1]">{college.name}</h1>
            ...
            <div className="flex flex-col gap-2 min-w-[220px]">
              ...
              {college.admission_email && (
                <a href={`mailto:${college.admission_email}`} className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold text-zinc-900 border border-zinc-900 transition-colors hover:bg-[#2563eb] hover:text-white hover:border-[#2563eb]">
      ...
      <div className="sticky top-[64px] z-30 w-full border-b border-border bg-background/95 backdrop-blur">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 flex gap-1 overflow-x-auto">
          {SECTIONS.map(s => (
            <a key={s.id} href={`#${s.id}`} className="px-3 py-3 text-sm font-semibold text-zinc-600 hover:text-[#2563eb] whitespace-nowrap transition-colors">
              {s.label}
            </a>
          ))}
        </div>
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 py-12 flex flex-col gap-14">
        ...
        <section id="overview" className="scroll-mt-32">
          <h2 className="text-2xl font-black text-zinc-900 mb-4">Overview</h2>
          {college.overview ? (
          ...
          {college.campus_facilities && college.campus_facilities.length > 0 && (
            <div className="mt-6">
              <h3 className="text-sm font-bold text-zinc-900 mb-3">Campus facilities</h3>
              <div className="flex flex-wrap gap-2">
                {college.campus_facilities.map((f: string) => (
                  <span key={f} className="flex items-center gap-1.5 text-sm font-medium px-3 py-1.5 rounded-full bg-[#2563eb]/5 text-zinc-700 border border-border">
        ...
        <section id="admission" className="scroll-mt-32">
          <h2 className="text-2xl font-black text-zinc-900 mb-4">Admission</h2>
          <div className="grid md:grid-cols-3 gap-6">
            <div className="md:col-span-2">
              ...
              {college.accepts_exams && (
                <p className="text-sm font-semibold text-zinc-700 mt-4">Accepts: <span className="text-[#2563eb]">{college.accepts_exams}</span></p>
              )}
            </div>
            <div className="rounded-xl border border-border bg-white p-5 flex flex-col gap-3 h-fit">
              {college.admission_email && (
                <a href={`mailto:${college.admission_email}`} className="flex items-center gap-2 text-sm text-zinc-700 hover:text-[#2563eb]">
                  <Mail className="h-4 w-4 shrink-0" /> {college.admission_email}
                </a>
              )}
              {college.phone && (
                <a href={`tel:${college.phone}`} className="flex items-center gap-2 text-sm text-zinc-700 hover:text-[#2563eb]">
        ...
        <section id="courses" className="scroll-mt-32">
          <h2 className="text-2xl font-black text-zinc-900 mb-4">Courses & seats</h2>
                  ...
                  <tbody>
                    {courses.map(c => (
                      <tr key={c.id} className="border-t border-border">
                        ...
                        <td className="px-4 py-3 text-zinc-600">{c.duration || "—"}</td>
                        <td className="px-4 py-3 text-zinc-600">{c.eligibility || "—"}</td>
                        <td className="px-4 py-3 text-zinc-600">{c.seats ?? "—"}</td>
                        <td className="px-4 py-3 text-zinc-600">{c.cutoff_general || "—"}</td>
                        <td className="px-4 py-3 text-zinc-600">{c.fees_total_lpa ?? "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <section id="placements" className="scroll-mt-32">
          <h2 className="text-2xl font-black text-zinc-900 mb-4">Placements{college.placement_year ? ` (${college.placement_year})` : ""}</h2>
          {hasPlacements ? (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
              {[
                ...
                <div key={label} className="rounded-xl border border-border bg-white p-5 flex flex-col gap-2">
              ...
              {college.top_recruiters && (
                <div className="col-span-2 md:col-span-3 rounded-xl border border-border bg-white p-5">
                  <p className="text-xs font-semibold uppercase tracking-wide text-zinc-400 mb-2">Top recruiters</p>
                  <p className="text-sm text-zinc-700">{college.top_recruiters}</p>
                </div>
              )}
            </div>
          ) : (
        ...
        <section id="reach" className="scroll-mt-32">
          <h2 className="text-2xl font-black text-zinc-900 mb-4">How to reach</h2>
