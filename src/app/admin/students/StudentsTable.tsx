  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: "#171b20", border: "1px solid rgba(255,255,255,0.13)" }}>
      ...
      <div className="overflow-x-auto">
            ...
            {filtered.map(row => (
              <tr key={row.id} className="border-t" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
                <td className="px-4 py-3">
                  <Link href={`/admin/students/${row.id}`} className="flex items-center gap-2 font-semibold hover:underline" style={{ color: "rgba(255,255,255,0.9)" }}>
                    {row.fullName}
                ...
                <td className="px-4 py-3" style={{ color: "rgba(255,255,255,0.6)" }}>{row.stream}</td>
                <td className="px-4 py-3" style={{ color: "rgba(255,255,255,0.6)" }}>{row.targetExam}</td>
                <td className="px-4 py-3" style={{ color: "rgba(255,255,255,0.6)" }}>{row.streak}</td>
        ...
        {filtered.length === 0 && (
          <p className="text-center py-10 text-sm" style={{ color: "rgba(255,255,255,0.3)" }}>No students match your search.</p>
        )}
      </div>
    </div>
  );
